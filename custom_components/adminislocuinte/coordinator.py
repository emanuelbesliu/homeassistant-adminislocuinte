"""Data Update Coordinator for Adminis Locuințe.
...
class AdminisLocuinteDataUpdateCoordinator(DataUpdateCoordinator[dict[str, Any]]):
    """Coordinator that fetches Adminis Locuințe data with resilient retry logic.
    ...
    async def _async_update_data(self) -> dict[str, Any]:
        """Fetch data from the Adminis Locuințe API.
        ...
        try:
            ...
            if self._is_auth_error(err):
                _LOGGER.error(
                    "Adminis Locuințe authentication failed - credentials may "
            ...
            _LOGGER.warning(
                "Adminis Locuințe data fetch failed (attempt %d). "
            ...
            raise UpdateFailed(
                f"Error communicating with Adminis Locuințe API: {err}"
            ) from err
    ...
    def _reset_retry(self) -> None:
        ...
        if self._retry_count > 0:
            _LOGGER.info(
                "Adminis Locuințe data fetch recovered after %d retries. "
